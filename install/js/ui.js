var UI = {
    ...
    Validation: 
	{
        ...
        requirements: function(notifyResult) 
		{
            ...
            Ajax.checkPermissions(function() {
                if ($('.folder-permissions .error').length) {
					notifyResult(false, 'لطفا دسترسی تمامی پوشه ها رو فیکس کنید');
                }
                else {
                        ...
                        if (result == '1') 
						{
							...
							Ajax.checkPhpVersion(function(result) {
								...
								if (result == '1') {
	                                ...
	                                notifyResult(false, 'یکتا سی ام اس آخرین نسخه پی اپ پی 5.3 را نیاز دارد');
								}
							});
                        }
                        else {
                            notifyResult(false, 'لطفا تمامی موارد مورد نیاز پی اچ پی را فعال کنید');
                        }
                    });
                }
            });
		...
		database: function(notifyResult) 
		{
			...
			if ( ! all_filled) {
				notifyResult(false, 'لطفا تمامی فیلد ها را پر کنید');
				return;
			}
			...
			Ajax.checkDbConnection(dbCMS, function(result) {
				...
				if (result != '1') {
					notifyResult(false, 'خطا در ارتباط سی ام اس با دیتابیس:<br />' + result);
				}
				else {
					Ajax.checkDbConnection(dbLogon, function(result) {
						
						if (result != '1') {
							notifyResult(false, 'خطا در ارتباط با دیتابیس:<br />' + result);
						}
						else {
							notifyResult(true);
						}
        ...
        realms: function(notifyResult) {			
			UI.confirm('<input type="text" id="superadmin" placeholder="نام کاربری که دسترسی مالک وارد نمایید" autofocus/>', 'تایید', function()
			{
