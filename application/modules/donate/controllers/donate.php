...
class Donate extends MX_Controller
{
	...
	public function index()
	{
		...
		$this->template->box("<span style='cursor:pointer;' onClick='window.location=\"" . $this->template->page_url . "ucp\"'>" . lang("ucp") . "</span> &larr; " . lang("donate_panel", "donate"), $output, true, "modules/donate/css/donate.css");
    ...
    public function request()
    {
		...
		$callback=$this->template->page_url . 'donate/verify';
        $Client = new SoapClient("http://www.jahanpay.org/webservice?wsdl");
        $Result = $Client->requestpayment($api, $Amount, $callback, 7, $Description);

        header("Location: http://www.jahanpay.org/pay_invoice/$Result");
    }
	...
	public function verify()
	{
		...
		if (strlen($Authority) > 4)
		{
			...
			if ($Result == 1)
			{
				...
				redirect($this->template->page_url . 'donate/success');
			} else {
