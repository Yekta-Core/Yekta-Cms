var Groups = {
	...
	remove: function(id, element)
	{
		...
		UI.confirm("آیا واقعا میخواید این گروه را حذف کنید ؟", "بله", function()
		{
	...
	removeAccount: function(name, field, groupId, isYourself)
	{
		...
		if(isYourself)
		{
			UI.confirm("<div style='margin-bottom:10px;font-size:16px;'>آیا مطمئن هستید که میخواهید این گروه را حذف کنید ؟</div><div style='margin-bottom:10px;font-size:10px;color:red;'>شما نمیتوانید دسترسی به پنل ادمین را لغو کنید</div>", "بله", proceed);
		}
		else
		{
			proceed();
	...
	addAccount: function(field, groupId)
	{
		...
		UI.confirm('<input type="text" id="add_member_name" placeholder="نام کاربری را وارد کنید" autofocus/>', 'افزودن', function()
		{
			...
			$.post(Config.URL + "admin/aclmanager/addMember/" + groupId, data, function(response)
			{
				if(response == "invalid")
				{
					UI.alert("حساب کاربری وجود ندارد");
