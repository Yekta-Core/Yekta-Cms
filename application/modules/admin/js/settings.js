var Settings = {
	...
	remove: function(id, element)
	{
		...
		{
			UI.alert("شما همیشه باید حداقل یک ریلم داشته باشید");
		}
		else
		{
			UI.confirm("آیا واقعا میخواهید این ریلم را حذف کنید ؟", "بله", function()
			{
	...
	saveWebsiteSettings: function()
	{
		...
		$.post(Config.URL + "admin/settings/saveWebsite", data, function(response)
		{
			if(response == "yes")
			{
				$("#website_ajax").html('تنظیمات ذخیره شد');
			}
			else
			{
				UI.alert(response);
	...
	saveSmtpSettings: function()
	{
		...
		$.post(Config.URL + "admin/settings/saveSmtp", data, function(response)
		{
			if(response == "yes")
			{
				$("#smtp_ajax").html('تنظیمات ذخیره شد');
			}
			else
			{
				UI.alert(response);
	...
	savePerformanceSettings: function()
	{
		...
		$.post(Config.URL + "admin/settings/savePerformance", data, function(response)
		{
			if(response == "yes")
			{
				$("#performance_ajax").html('تنظیمات ذخیره شد');
			}
			else
			{
				UI.alert(response);
	...
	toggleSource: function(id, field)
	{
		...
		{
			$(field).html("کد ویرایش سورس (پیشرفته)");
		...
		{
			$(field).html("ویرایش به صورت ساده");
