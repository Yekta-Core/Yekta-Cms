<section class="box big">
	...
	<form onSubmit="Groups.save(this, {$group.id}); return false" id="submit_form">
		...
		<label for="color" dir="rtl" align="right">رنگ گروه (optional)</label>
		...
		<span>
			<div class="memberList" dir="rtl" align="right">
				...
				{if $group.id == $guestId}
					آخرین بازدید که امضا کردن به طور خودکار به این گروه خواهد اختصاص داده می شود
				{elseif $group.id == $playerId}
					آخرین بازدید که در امضا به صورت خودکار به این گروه خواهد اختصاص داده می شود
				{else}
					{if $members}
					...
					<a href="javascript:void(0)" onClick="Groups.addAccount(this, {$group.id})" class="add">
						<img src="{$url}application/images/icons/add.png" />افزودن
					</a>
					<div class="clear"></div>
				{/if}
			</div>
		...
		<label for="roles" dir="rtl" align="right">
			<a href="javascript:void(0)" onClick="$('#visibility input[type=checkbox]').each(function(){ this.checked = true; });" style="float:right;display:block;">[انتخاب همه]</a>
			مجوز های دید
		</label>

		<div id="visibility" dir="rtl" align="right">
			{if $links}
