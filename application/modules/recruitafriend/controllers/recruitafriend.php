...
class Recruitafriend extends MX_Controller
{
	...
	{
		$this->template->setTitle("دعوت نامه");
		...
		$this->template->view($this->template->loadPage("page.tpl", array(
			"module" => "default", 
			"headline" => "<span style='cursor:pointer;' onClick='window.location=\"".$this->template->page_url."ucp\"'>پنل کاربری</span> &larr; دعوت نامه",
		...
		switch($this->canClaim($id))
		{
			...
			case 1:
				$this->db->query("INSERT INTO recruit_a_friend_claimed (recruiter, referral) VALUES (".$this->user->getId().", ".$id.")");
					...
					"module" => "default", 
					"headline" => "<span style='cursor:pointer;' onClick='window.location=\"".$this->template->page_url."ucp\"'>UCP</span> &rarr; Referrals",
					"content" => "شما پاداش دریافت کردید".$this->config->item('recruit_reward_amount')." ".strtoupper($this->config->item('recruit_reward_type'))." برای معرفی دوستان
					<script>
					setTimeout(function(){
			...
			case 2:
				$this->template->view($this->template->loadPage("page.tpl", array(
				"module" => "default", 
				"headline" => "<span style='cursor:pointer;' onClick='window.location=\"".$this->template->page_url."ucp\"'>پنل کاربری</span> &rarr; دعوت نامه",
				"content" => "شما از این اکانت پاداش دریافت کرده اید")));
				break;
			case 3:
				$this->template->view($this->template->loadPage("page.tpl", array(
					"module" => "default", 
					"headline" => "<span style='cursor:pointer;' onClick='window.location=\"".$this->template->page_url."ucp\"'>UCP</span> &rarr; Referrals",
					"content" => "شما نمیتوانید از این اکانت پاداش دریافت کنید")));
				break;
			default:
				$this->template->view($this->template->loadPage("page.tpl", array(
					"module" => "default", 
					"headline" => "<span style='cursor:pointer;' onClick='window.location=\"".$this->template->page_url."ucp\"'>UCP</span> &rarr; Referrals",
					"content" => "خطایی رخ داده")));
					break;
		}
	}
}