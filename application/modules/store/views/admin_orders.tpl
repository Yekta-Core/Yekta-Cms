<section class="box big">
	<h2>
		...
		سفارشات ناموفق در هفته گذشته
	</h2>

	<span>
		در این قسمت میتوانید سفارشات ناموفق 7 روز گذشته را مشاهده کنید
	</span>
				...
				<li>
					<table width="100%">
						<tr>
							...
							<td>
								<a data-tip="{foreach from=$failed_log.json item=item}{$item.itemName} به {$item.characterName}<br />{/foreach}">{count($failed_log.json)} آیتم ها</a>
							</td>
							
							{if hasPermission("canRefundOrders")}
								<td style="text-align:right;">
									<a class="nice_button" href="javascript:void(0)" onClick="Orders.refund({$failed_log.id}, this)">پرداخت مجدد</a>
								</td>
							{/if}
						</tr>
					</table>
...
<section class="box big">
	<h2>
		<img src="{$url}application/themes/admin/images/icons/black16x16/ic_list.png"/>
		لیست 10 سفارش موفق ( آخرین سفارشات )
	</h2>

	<form style="margin-top:0px;" onSubmit="Orders.search('successful'); return false">
		<input type="text" name="search_successful" id="search_successful" placeholder="جستجو بر اساس نام کاربری" style="width:90%;margin-right:5px;"/>
