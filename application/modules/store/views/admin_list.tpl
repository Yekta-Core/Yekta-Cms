	...
	<li>
		<table width="100%">
			<tr>
				...
				<td>
					<a data-tip="{foreach from=$order_log.json item=item}{$item.itemName} به {$item.characterName}<br />{/foreach}">{count($order_log.json)} آیتم ها</a>
				</td>

				{if $order_log.completed == '0' && hasPermission("canRefundOrders")}
					<td style="text-align:right;">
						<a class="nice_button" href="javascript:void(0)" onClick="Orders.refund({$order_log.id}, this)">پراخت مجدد</a>
					</td>
				{/if}
			</tr>
		</table>
